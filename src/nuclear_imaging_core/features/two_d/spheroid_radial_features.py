...
def get_nuc_posn_in_spheroid_2d(inp_nucdata_df,sphdata_df,nuc_labels,spheroid_labels):
    ...
    for si in range(len(unique_sphs)):
        ...
        for index, row in nuc_in_spheroid.iterrows():
            ceny, cenx = int(row['ceny']), int(row['cenx'])

            # Find distance from nucleus to spheroid mask boundary in the 2D plane
            z_plane_mask = np.copy(spheroid_mask)
            # Find the edge of the 2D mask
            z_plane_edges = morphology.binary_erosion(z_plane_mask) ^ z_plane_mask
            distance_to_boundary = ndi.distance_transform_edt(~z_plane_edges)[ceny, cenx]
...
def assign_shell_nucleii_2d(inp_nucdata_df,sphdata_df,nuc_labels,spheroid_labels,
    ...
    for si in range(len(unique_sphs)):
        ...
        for i in range(nbins):
            ...
            coords = np.array([np.array([nucdata_df.iloc[n]["ceny"].astype(int),
...
def assign_outward_nucleii_2d(inp_nucdata_df,sphdata_df,nuc_labels,spheroid_labels,
                   nbins:int = 20, maxdist:float = 400):
...
def process_radial(nucdata_df,sphdata_df,nuc_labels,spheroid_labels,
    ...
    nucdata_df = assign_shell_nucleii_2d(nucdata_df,sphdata_df,nuc_labels,spheroid_labels,
                                      nbins,maxdist_shell)

    print("done shell nucleii",nucdata_df.values.shape)

    nucdata_df = assign_outward_nucleii_2d(nucdata_df,sphdata_df,nuc_labels,spheroid_labels,
                                      nbins,maxdist_outward)

    print("done outward nucleii",nucdata_df.values.shape)

    nucdata_df = get_nuc_posn_in_spheroid_2d(nucdata_df,sphdata_df,nuc_labels,spheroid_labels)
    ...
    for col in nuc_col_list:
        ...
        ### get nucs
            ...
            if nucdata_df[nucdata_df["shell_bin_ref_sph"]==i][col].values.size == 0:
                ...
                print(col,i,nucdata_df[nucdata_df["shell_bin_ref_sph"]==i][col].values)
                # print(nucdata_df[nucdata_df["shell_bin_ref_sph"]==i][col].values)
