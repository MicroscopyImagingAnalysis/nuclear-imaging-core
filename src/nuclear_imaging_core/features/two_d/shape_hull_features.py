...
##    print(foo)
    ...
    ceny, cenx = (float(foo['centroid-0'][0]),float(foo['centroid-1'][0]))
    edge = np.subtract(bw * 1, erosion(bw) * 1)
...
##        surfpoints[i,2] = 0.0 #z_step_size*(boundary_z[i]+0.5)
    ...
    ### now compute 2D hull
    bulk_y,bulk_x = [np.where(binary_mask>0)[0],np.where(binary_mask>0)[1]]
    ...
    del surfpoints, bulkpoints, surfhull, edge, boundary_x, boundary_y, bulk_x, bulk_y
    return feat
...
def measure_calliper(binary_mask: np.ndarray,angular_resolution:int = 10):
    ...
    max_calliper, min_calliper = measure_2d_calliper(binary_mask, angular_resolution)
    feat = {
        "2d_max_calliper": max_calliper,
        "2d_min_calliper": min_calliper
    }

    del binary_mask, max_calliper, min_calliper
