...
##        exit(0)
    ...
    if np.max(subreg) <= 1:
        # Compute the fraction of saturated pixels
        saturated_fraction = np.sum(subreg >= 0.99) / subreg.size
        feat.update({"saturated_fraction": saturated_fraction})
...
def spatialIntensity(full_raw: np.ndarray,binary_mask: np.ndarray,
                      ...
                      nbins: int=10, pixel_size:float = 0.35,z_step_size:float=1.5):
...
##    print(foo)
    ...
    boundary_points = np.zeros([boundary_x.shape[0],3],np.float64)
    for i in range(boundary_x.shape[0]):
        ...
        boundary_points[i,2] = z_step_size*(boundary_z[i]+0.5)
    ...
    for i in range(len(uniq_labels)):
        dist_f_b = np.sqrt(
            ...
            np.square(boundary_points[:,2]-peak_points[i,2])
        )
